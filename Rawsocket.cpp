...
BOOL CWizSyncSocket::Connect(LPCTSTR lpszHostAddress, UINT nHostPort, int iType)
{
    ...
    {
        // Convert LPCWSTR (wide string) to std::string (narrow string)
        int bufferSize = WideCharToMultiByte(CP_UTF8, 0, lpszHostAddress, -1, NULL, 0, NULL, NULL);
        std::string hostname(bufferSize, 0);
        WideCharToMultiByte(CP_UTF8, 0, lpszHostAddress, -1, &hostname[0], bufferSize, NULL, NULL);

        m_strHostname = hostname;  // Now we can assign the converted narrow string to m_strHostname
    }
	// Create ? socket
	if (m_hSocket == INVALID_SOCKET)
	{
